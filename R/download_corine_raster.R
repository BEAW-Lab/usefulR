...
download_corine_raster <- function(
  ...
  resolve_access_token <- function(token_value) {
    ...
    if (grepl("^\\{", trimmed)) {
      ...
      jwt_assertion <- jose::jwt_encode_sig(
        claim = claim_set,
        key = private_key
      )
      ...
      token_response <- curl::curl_fetch_memory(
      ...
      if (
        is.null(token_json$access_token) || !nzchar(token_json$access_token)
      ) {
        stop(
          "The CLMS token endpoint did not return an access token.",
          call. = FALSE
        )
      }
  ...
  if (
    !is.character(download_url) ||
      length(download_url) != 1L ||
      is.na(download_url) ||
      download_url == ""
  ) {
    stop(
      "The CLMS API did not provide a download URL for the CORINE archive.",
      call. = FALSE
    )
  }
  ...
  tif_candidates <- zip_listing$Name[grepl(
    "\\.tif$",
    zip_listing$Name,
    ignore.case = TRUE
  )]
  target_tif <- tif_candidates[
    basename(tif_candidates) == "U2018_CLC2018_V2020_20u1.tif"
  ]
  ...
  if (length(target_tif) == 0L || is.na(target_tif)) {
    stop(
      "No GeoTIFF file was found inside the downloaded CORINE archive.",
      call. = FALSE
    )
  }
  ...
  if (!file.exists(extracted_file)) {
    stop(
      "The expected CORINE GeoTIFF could not be extracted from the archive.",
      call. = FALSE
    )
  }
