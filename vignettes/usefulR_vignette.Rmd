...
* calculate urban score using `extract_land_cover.R` (= proportion of urban / impervious land cover around a given coordinate)
...
pak::pkg_install("BEAW-Lab/usefulR") # recommended
...
If you install from a manually downloaded GitHub ZIP instead of using `pak` or `remotes`, dependencies may not be installed automatically.
...
set.seed(7)
coor <- data.frame(
  lon = runif(n = 50, min = -6, max = -3),
  lat = runif(n = 50, min = 39, max = 42),
  x_var1 = rnorm(n = 50, mean = 0, sd = 5)
) # this variable is made up, just a dummy to run a model later
head(coor)
```
...
uses [ESA WorldCover 2021 data](https://worldcover2021.esa.int/) by default. The same function can also be used
with CORINE Land Cover rasters by setting `dataset = "corine"`.
...
usefulR::download_ESAWC21_raster_vignette(
...
usefulR::extract_land_cover(
  coords = data.frame(lon = -4.5, lat = 41.00), # (longitude, latitude) the order is important
  raster_path = path_to_raster, # if the vignette is built. Name of the downloaded file
  radius_m = 1500, # buffer radius in meters
  dataset = "esawc21"
) # 'esawc21' uses class 50 (built-up) by default.
```
...
data_land_cover <- usefulR::extract_land_cover(
  coords = coor |> dplyr::select(lon, lat),
...
```{r, include = T, eval = FALSE}
raster_esa <- terra::rast('./path/to/directory/where/raster/is/saved/LC_raster.tif')
plot(raster_esa) # plot the raster to double check you approximately have the geographical coverage you need

extract_land_cover(
  coords = coor |> dplyr::select(lon, lat),
  raster_path = './path/to/directory/where/raster/is/saved/LC_raster.tif',
  radius_m = 1500,
  dataset = "esawc21"
)
...
If you want to use CORINE Land Cover instead, first download the hosted CORINE raster with
`download_corine_raster()`, then provide that raster and set `dataset = "corine"`.
By default the function will treat artificial-surface classes (111, 112, 121, 122, 123, 124, 131, 132, 133, 141, 142)
as urban land cover:
```{r, include = T, eval = FALSE}
# first download the hosted CORINE raster to your chosen local folder
...
# then use the downloaded raster in extract_land_cover()
path_to_raster_corine <- file.path(vignette_dir, "CLC_CORINE_2018.tif")

extract_land_cover(
  coords = coor |> dplyr::select(lon, lat),
  raster_path = path_to_raster_corine,
  radius_m = 1500,
  dataset = "corine"
)
```
...
usefulR::download_viirs_raster(
  local_dir = vignette_dir, # put the directory where you want to save the file
...
```{r, include = T, eval = TRUE}
data_alan <- usefulR::extract_alan(
  coords = coor |> dplyr::select(lon, lat),
...
head(data_alan)
```
You can now then merge the calculation of ALAN to the dataset for analysis:
```{r, include = T}
data <- dplyr::left_join(
  x = data00,
  y = data_alan |> dplyr::select(lon, lat, alan, alan_log),
  by = c('lon', 'lat')
)
...
data$response <- 3.5 *
  data$percent_urban +
  rnorm(n = nrow(data), mean = 0, sd = 15)
```

Now we run the model to explain variation in the response variable including `percent_urban` and `x_var1` as fixed effects.
```{r model, include = T}
model <- lm(response ~ percent_urban + alan + x_var1, data = data)
summary(model)
```

Once we have the model, we can produce a table of results with `table_results`:
```{r table of results, include = T, eval = T}
usefulR::table_results(
  label_list = list(
    `(Intercept)` = "Intercept",
    `x_var1` = "x_var1",
    `percent_urban` = '% of impervious surface',
...
gt::gtsave(table01, "./tables/table_01.html")
```
