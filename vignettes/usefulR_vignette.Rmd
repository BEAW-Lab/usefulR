...
extract_land_cover(coords = data.frame(lon = -4.5, lat = 41.00), # (longitude, latitude) the order is important
                   ...
                   dataset = "esawc21")    # 'esawc21' uses class 50 (built-up) by default.
...
set.seed(8)
data$response <- 3.5 * data$percent_land_cover + rnorm(n = nrow(data), mean = 0, sd = 15)
```

Now we run the model to explain variation in the response variable including `percent_land_cover` and `x_var1` as fixed effects.
```{r model, include = T}
model <- lm(response ~ percent_land_cover + alan + x_var1, data = data)
summary(model)
```
...
table_results(
  label_list = list(
    `(Intercept)` = "Intercept",
    `x_var1` = "x_var1",
    `percent_land_cover` = '% of urban land cover',
